...
namespace LotTraceApp.Services
{
    ...
    new Dictionary<string, List<BackwardParentCandidate>>(StringComparer.OrdinalIgnoreCase);
            ...
            {
                var startDLocalEdgeKeys = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                ...
                foreach (var candidate in startDrumcanCandidates)
                {
                    ...
                    var rawChild = candidate.ChildNode;
                    rawChild.Depth = 0;
                    rawChild.ParentKey = null;
                    rawChild.StartDateLabel = ResolveStartDateLabel(rawChild, null);

                    var childNode = GetOrAddNode(nodeMap, rawChild);
                    EnsureNodeRegistered(result, childNode);
                    EnsureRootRegistered(result, childNode);

                    // START-D の child 側も、通常始点と同じ開始ノード集合に載せる。
                    // ここでは重複削除しない。
                    startNodes.Add(childNode);


                    string childLaneKey = GetNodeMergeKey(childNode);
                    if (!string.IsNullOrWhiteSpace(childLaneKey) && laneStartNodeKeys.Add(childLaneKey))
                    {
                        laneStartNodes.Add(childNode);
                    }

                    var rawParent = candidate.Node;
                    rawParent.Depth = childNode.Depth + 1;
                    ...
                    string localEdgeKey = BuildBackwardLocalEdgeKey(parentNode, childNode, candidate);
                    if (!string.IsNullOrWhiteSpace(localEdgeKey) &&
                        !startDLocalEdgeKeys.Add(localEdgeKey))
                    {
                        continue;
                    }
                    ...
                    // StartD は child 側から BFS を広げない。
                    // 必要なら parent 側だけを次 current として通常 BFS に渡す。
                    if (linkAdded)
                    {
            ...
            while (queue.Count > 0)
            {
                ...
                DumpBackwardParentCandidatesExpanded(current, parentCandidates);
            ...
            // バック探索結果 → 枝構造化
            // ------------------------------------------------------------
            var backwardLaneBuildResult = BuildBackwardDisplayLaneNodes_SimpleDebug(result, startNodes);
            //var backwardLaneBuildResult = BuildBackwardDisplayLaneNodes(result, startNodes);
    ...
    IList<OccupiedLotGroupRange> occupiedGroups)
{
    ...
    List<DisplayLaneNode> laneNodes)
        ...
        private ForwardDisplayLaneBuildResult BuildBackwardDisplayLaneNodes_SimpleDebug(
    TraceResult result,
    List<ProductionResultNode> startNodes)
        {
            ...
            // 追加：始点→Lv1 は Roots グループ全体で RelationKey を共有評価する
            //var seenRelationKeysAcrossRoots =
            ...
            foreach (var root in roots)
            {
                ...
                List<BackwardParentCandidate> candidates = null;

                if (_currentBackwardCandidatesByChildNodeKey != null &&
                    _currentBackwardCandidatesByChildNodeKey.Count > 0)
                {
                    string childNodeKey = GetBackwardDisplayChildNodeKey(root, null);

                    if (string.IsNullOrWhiteSpace(childNodeKey))
                    {
                        string mergeKey = GetNodeMergeKey(root);
                        if (!string.IsNullOrWhiteSpace(mergeKey))
                        {
                            childNodeKey = mergeKey;
                        }
                        else if (!string.IsNullOrWhiteSpace(root.ControlMasterKey))
                        {
                            childNodeKey = root.ControlMasterKey.Trim();
                        }
                        else if (!string.IsNullOrWhiteSpace(root.LotNumber))
                        {
                            childNodeKey = root.LotNumber.Trim();
                        }
                    }

                    if (!string.IsNullOrWhiteSpace(childNodeKey))
                    {
                        _currentBackwardCandidatesByChildNodeKey.TryGetValue(childNodeKey, out candidates);
                    }
                }

                if (candidates != null && candidates.Count > 0)
                {
                    var filteredCandidates = new List<BackwardParentCandidate>();
                    ...
                    foreach (var candidate in candidates)
                    {
                        ...
                        //if (!seenRelationKeysAcrossRoots.Add(candidate.RelationKey))
                        //    continue;
                ...
                var validCandidates = candidates == null
                    ? new List<BackwardParentCandidate>()
                    : candidates
                        .Where(x => x != null && x.Node != null)
                        .ToList();
                ...
                var rootDisplayNode = new DisplayLaneNode
                {
                    DisplayNodeKey = BuildDisplayLaneNodeKey(root, 0, rootY),
                    MergeKey = GetNodeMergeKey(root),
                    ParentDisplayNodeKey = null,
                    SourceNode = root,
                    XLevel = 0,
                    ...
                    RootGroupHeight = 1,
                    ChildBranchHeight = 0,
                    OccupiedFirstY = rootY,
                    OccupiedLastY = rootY
                };

                displayNodes.Add(rootDisplayNode);
                ...
                foreach (var candidate in validCandidates)
                {
                    ...
                    var parentDisplayNode = new DisplayLaneNode
                    {
                        DisplayNodeKey = BuildDisplayLaneNodeKey(parentNode, 1, parentY),
                        MergeKey = GetNodeMergeKey(parentNode),
                        ParentDisplayNodeKey = rootDisplayNode.DisplayNodeKey,
                        ...
                        RootGroupHeight = 1,
                        ChildBranchHeight = 0,
                        OccupiedFirstY = parentY,
                        OccupiedLastY = parentY
                    };

                    displayNodes.Add(parentDisplayNode);
                ...
                {
                    // root の最終座標を親群の先頭Yへ合わせる
                    // DisplayNodeKey は座標込みのため、ここで最終キーへ更新する
                    string oldRootDisplayNodeKey = rootDisplayNode.DisplayNodeKey;
                    string newRootDisplayNodeKey = BuildDisplayLaneNodeKey(root, 0, firstY);

                    rootDisplayNode.YLane = firstY;
                    rootDisplayNode.DisplayNodeKey = newRootDisplayNodeKey;
                    rootDisplayNode.ChildBranchHeight = createdChildCount;
                    rootDisplayNode.OccupiedFirstY = firstY;
                    rootDisplayNode.OccupiedLastY = lastY;
                    rootDisplayNode.SubtreeLaneSpan = Math.Max(1, lastY - firstY + 1);

                    if (!string.Equals(oldRootDisplayNodeKey, newRootDisplayNodeKey, StringComparison.Ordinal))
                    {
                        foreach (var node in displayNodes)
                        {
                            if (node == null)
                            ...
                            if (string.Equals(node.ParentDisplayNodeKey, oldRootDisplayNodeKey, StringComparison.Ordinal))
                            {
                                node.ParentDisplayNodeKey = newRootDisplayNodeKey;
                            }
                            ...
                            foreach (var edge in node.OutgoingEdges)
                            {
                                ...
                                if (string.Equals(edge.ParentDisplayNodeKey, oldRootDisplayNodeKey, StringComparison.Ordinal))
                                {
                                    edge.ParentDisplayNodeKey = newRootDisplayNodeKey;
                                }

                                if (string.Equals(edge.ChildDisplayNodeKey, oldRootDisplayNodeKey, StringComparison.Ordinal))
                                {
                                    edge.ChildDisplayNodeKey = newRootDisplayNodeKey;
                                }
                            }
                        }
                    }
    ...
    //        string currentVisitKey = BuildBackwardNodeVisitKey(currentNode);
    ...
    //                    ParentDisplayNodeKey = parentDisplayNode == null
    //                        ? null
    ...
    //            var parentRelationGroups = GetBackwardRelationGroups(
    //                currentNode,
    //                _currentBackwardCandidatesByChildNodeKey);

    //            if (parentRelationGroups == null || parentRelationGroups.Count == 0)
    //            {
    ...
    //            foreach (var relationGroup in parentRelationGroups)
    //            {
    //                if (relationGroup == null || relationGroup.Count == 0)
    //                    continue;

    //                var representativeCandidate = relationGroup
    //                    .Where(x => x != null && x.Node != null)
    //                    .OrderBy(x => NormalizeBackwardRelationKey(x), StringComparer.OrdinalIgnoreCase)
    //                    .FirstOrDefault();

    //                if (representativeCandidate == null || representativeCandidate.Node == null)
    //                    continue;

    //                var parentNode = representativeCandidate.Node;
    ...
    //                if (!TryAddBackwardDisplayEdge(
    //                    currentDisplayNode,
    //                    parentDisplayNodeCreated,
    ...
    //    }
        ...
        private List<BackwardParentCandidate> BuildBackwardParentCandidatesForBranch(
    ProductionResultNode currentNode)
        {
            ...
            if (relationGroups == null || relationGroups.Count == 0)
                return result;
            ...
            foreach (var relationGroup in relationGroups)
            {
                ...
                foreach (var candidate in relationGroup)
                {
                    if (candidate != null)
                    {
        ...
        private string BuildBackwardNodeVisitKey(ProductionResultNode node)
        {
            if (node == null)
                return null;

            string mergeKey = GetNodeMergeKey(node);
            if (!string.IsNullOrWhiteSpace(mergeKey))
                return mergeKey;

            return GetBackwardVisitedKey(node);
        }

        private bool TryAddBackwardDisplayEdge(
    DisplayLaneNode childDisplayNode,
    DisplayLaneNode parentDisplayNode,
    BackwardParentCandidate candidate,
    int childIndex)
        {
            if (childDisplayNode == null || parentDisplayNode == null || candidate == null)
                return false;

            string edgeIdentityKey = candidate.RelationKey;
            if (string.IsNullOrWhiteSpace(edgeIdentityKey))
            {
                edgeIdentityKey = BuildBackwardRelationKey(candidate);
            }

            bool edgeExists = childDisplayNode.OutgoingEdges.Any(e =>
                e != null &&
                !string.IsNullOrWhiteSpace(e.EdgeIdentityKey) &&
                string.Equals(e.EdgeIdentityKey, edgeIdentityKey, StringComparison.OrdinalIgnoreCase));

            if (edgeExists)
                return false;

            childDisplayNode.OutgoingEdges.Add(new DisplayLaneEdge
            {
                EdgeIdentityKey = edgeIdentityKey,
                ParentDisplayNodeKey = childDisplayNode.DisplayNodeKey,
                ChildDisplayNodeKey = parentDisplayNode.DisplayNodeKey,
                ParentMergeKey = childDisplayNode.MergeKey,
                ChildMergeKey = parentDisplayNode.MergeKey,
                FromXLevel = childDisplayNode.XLevel,
                ToXLevel = parentDisplayNode.XLevel,
                FromYLane = childDisplayNode.YLane,
                ToYLane = parentDisplayNode.YLane,
                ChildIndex = childIndex,
                LotGroupKey = null
            });
    ...
    BackwardParentCandidate candidate)
        {
            return "BACKWARD_LOCAL|" + BuildBackwardRelationKey(candidate);
        }
    ...
    BackwardParentCandidate candidate)
        ...
        /// <summary>
        /// RelationKeyをリポジトリに移管したので不要メソッド
        /// 接続を全て切ってから削除
        /// </summary>
        /// <param name="candidate"></param>
        /// <returns></returns>
        private string NormalizeBackwardRelationKey(BackwardParentCandidate candidate)
        {
            ...
            string relationKey = BuildBackwardRelationKey(candidate);
            return string.IsNullOrWhiteSpace(relationKey)
                ? string.Empty
                : relationKey.Trim();
        }
    ...
    BackwardParentCandidate candidate)
        {
            if (backwardCandidatesByChildNodeKey == null)
                return;

            if (childNode == null || candidate == null || candidate.ChildNode == null)
                return;
        ...
        private string GetBackwardDisplayChildNodeKey(
            ...
            BackwardParentCandidate candidate)
        {
            ProductionResultNode relationChildNode = candidate == null
                ? null
                : candidate.ChildNode;

            string mergeKey = GetNodeMergeKey(relationChildNode ?? childNode);
            if (!string.IsNullOrWhiteSpace(mergeKey))
                return mergeKey;

            var keySourceNode = relationChildNode ?? childNode;

            if (keySourceNode != null && !string.IsNullOrWhiteSpace(keySourceNode.ControlMasterKey))
                return keySourceNode.ControlMasterKey.Trim();

            if (keySourceNode != null && !string.IsNullOrWhiteSpace(keySourceNode.LotNumber))
                return keySourceNode.LotNumber.Trim();
        ...
        private string BuildBackwardRelationKey(BackwardParentCandidate candidate)
        {
            ...
            var relationParentNode = candidate.Node;
            var relationChildNode = candidate.ChildNode;

            string parentMergeKey = GetNodeMergeKey(relationParentNode);
            if (string.IsNullOrWhiteSpace(parentMergeKey))
            {
                parentMergeKey = relationParentNode == null
                    ? string.Empty
                    : (relationParentNode.ControlMasterKey
                        ?? relationParentNode.LotNumber
                        ?? string.Empty);
            }

            string childLot;
            if (!string.IsNullOrWhiteSpace(candidate.ChildNode.LotNumber))
            {
                childLot = candidate.ChildNode.LotNumber.Trim().ToUpperInvariant();
            }
            else if (relationChildNode != null &&
                     !string.IsNullOrWhiteSpace(relationChildNode.LotNumber))
            {
                childLot = relationChildNode.LotNumber.Trim().ToUpperInvariant();
            }
            else
            {
                childLot = string.Empty;
            }

                    return string.Join("|",
            childLot,
            parentMergeKey ?? string.Empty);
        }
    ...
    Dictionary<string, List<BackwardParentCandidate>> backwardCandidatesByChildNodeKey)
        {
            ...
            var grouped = candidates
                .Where(x => x != null && x.Node != null && !string.IsNullOrWhiteSpace(x.RelationKey))
                .GroupBy(x => x.RelationKey, StringComparer.OrdinalIgnoreCase)
                .OrderBy(g => g.Key, StringComparer.OrdinalIgnoreCase);
    ...
    List<DisplayLaneNode> displayNodes)
        ...
        private bool IsBackwardExcludedParentMasterKey(string masterKey)
        {
            if (string.IsNullOrWhiteSpace(masterKey))
                return false;
    ...
    IList<BackwardParentCandidate> parentCandidates)
        ...
        private void DumpBackwardDisplayCandidateKeyProbe(
    ProductionResultNode childNode,
    ProductionResultNode parentNode,
    BackwardParentCandidate candidate,
    string childNodeKey,
    string status)
        {
            try
            {
                ...
                string path = Path.Combine(dir, "LotTrace_Debug_BackwardDisplayCandidateKeyProbe.csv");
                bool exists = File.Exists(path);
                ...
                using (var sw = new StreamWriter(path, true, Encoding.UTF8))
                {
                    if (!exists)
                    {
                        sw.WriteLine(
                            ...
                            + "ChildLotNumber,"
                            + "ChildRouteSystem,"
                            + "ChildInputSourceType,"
                            + "ChildInputSlotNo,"
                            + "CandidateChildMergeKey,"
                            + "CandidateChildMasterKey,"
                            + "CandidateChildLotNumber,"
                            + "CandidateRelationKey,"
                            + "CandidateLocalEdgeKey,"
                            + "ParentMergeKey,"
                            + "ParentMasterKey,"
                            + "ParentLotNumber,"
                            + "ParentRouteSystem,"
                            + "ParentInputSourceType,"
                            + "ParentInputSlotNo");
                    }

                    var candidateChild = candidate == null ? null : candidate.ChildNode;

                    sw.WriteLine(string.Join(",",
                        EscapeCsv(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")),
                        EscapeCsv(status),
                        EscapeCsv(childNodeKey),
                        EscapeCsv(GetNodeMergeKey(childNode)),
                        EscapeCsv(childNode == null ? null : childNode.ControlMasterKey),
                        EscapeCsv(childNode == null ? null : childNode.LotNumber),
                        EscapeCsv(childNode == null ? null : childNode.RouteSystem),
                        EscapeCsv(childNode == null ? null : childNode.InputSourceType),
                        EscapeCsv(childNode == null || !childNode.InputSlotNo.HasValue ? null : childNode.InputSlotNo.Value.ToString()),
                        EscapeCsv(GetNodeMergeKey(candidateChild)),
                        EscapeCsv(candidateChild == null ? null : candidateChild.ControlMasterKey),
                        EscapeCsv(candidateChild == null ? null : candidateChild.LotNumber),
                        EscapeCsv(candidate == null ? null : candidate.RelationKey),
                        EscapeCsv(BuildBackwardLocalEdgeKey(parentNode, childNode, candidate)),
                        EscapeCsv(GetNodeMergeKey(parentNode)),
                        EscapeCsv(parentNode == null ? null : parentNode.ControlMasterKey),
                        EscapeCsv(parentNode == null ? null : parentNode.LotNumber),
                        EscapeCsv(parentNode == null ? null : parentNode.RouteSystem),
                        EscapeCsv(parentNode == null ? null : parentNode.InputSourceType),
        ...
        private void DumpBackwardDisplayLookupKeyProbe(
    ProductionResultNode rootNode,
    string childNodeKey,
    bool found,
    int candidateCount,
    string status)
        {
            try
            {
                ...
                string path = Path.Combine(dir, "LotTrace_Debug_BackwardDisplayLookupKeyProbe.csv");
                bool exists = File.Exists(path);
                ...
                using (var sw = new StreamWriter(path, true, Encoding.UTF8))
                {
                    ...
                    sw.WriteLine(string.Join(",",
                        ...
                        EscapeCsv(status),
                        EscapeCsv(found ? "1" : "0"),
                        EscapeCsv(candidateCount.ToString()),
                        EscapeCsv(childNodeKey),
                        EscapeCsv(GetNodeMergeKey(rootNode)),
                        EscapeCsv(rootNode == null ? null : rootNode.ControlMasterKey),
                        EscapeCsv(rootNode == null ? null : rootNode.LotNumber),
                        EscapeCsv(rootNode == null ? null : rootNode.RouteSystem),
                        EscapeCsv(rootNode == null ? null : rootNode.InputSourceType),
                        EscapeCsv(rootNode == null || !rootNode.InputSlotNo.HasValue ? null : rootNode.InputSlotNo.Value.ToString())
                    ));
                }
            }
            catch
