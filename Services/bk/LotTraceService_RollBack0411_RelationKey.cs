        ...
        private TraceResult TraceBackward(TraceSearchParameters p)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                foreach (var candidate in parentCandidates)
                {
                    ...
                    // 枝構造用:
                    // backward は ParentNodes 直読みではなく、RelationKey 文脈を保持して組み立てる
    ...
    Dictionary<string, DisplayLaneNode> displayNodeByBackwardKey)
        {
            ...
            try
            {
                ...
                // ------------------------------------------------------------
                // backward では同じ元 Node の DisplayLaneNode は 1 回だけ作る
                // ------------------------------------------------------------
                if (!string.IsNullOrWhiteSpace(backwardNodeKey))
                {
                ...
                var parentCandidates = GetBackwardOrderedParentCandidates(currentNode);
                ...
                {
                    if (parentCandidate == null || parentCandidate.Node == null)
                        continue;

                    var parentNode = parentCandidate.Node;
                    string relationKey = NormalizeBackwardRelationKey(parentCandidate);
    ...
    BackwardParentCandidate candidate)
        {
            ...
            if (childNode == null || candidate == null || normalizedParentNode == null)
                return;

            string childNodeKey = BuildBackwardDisplayChildNodeKey(childNode);
            if (string.IsNullOrWhiteSpace(childNodeKey))
                return;

            List<BackwardParentCandidate> list;
            if (!_currentBackwardCandidatesByChildNodeKey.TryGetValue(childNodeKey, out list))
            {
                list = new List<BackwardParentCandidate>();
                _currentBackwardCandidatesByChildNodeKey[childNodeKey] = list;
            }
        ...
        private List<BackwardParentCandidate> GetBackwardOrderedParentCandidates(
            ProductionResultNode childNode)
        {
            ...
            string childNodeKey = BuildBackwardDisplayChildNodeKey(childNode);
            if (!string.IsNullOrWhiteSpace(childNodeKey) &&
                _currentBackwardCandidatesByChildNodeKey != null)
            {
                List<BackwardParentCandidate> candidates;
                if (_currentBackwardCandidatesByChildNodeKey.TryGetValue(childNodeKey, out candidates) &&
                    candidates != null &&
                    candidates.Count > 0)
                {
                    var groupedByRelation = candidates
    .Where(x => x != null && x.Node != null)
    .GroupBy(
        x => NormalizeBackwardRelationKey(x),
        StringComparer.OrdinalIgnoreCase)
    .OrderBy(g => g.Key, StringComparer.OrdinalIgnoreCase);

                    foreach (var relationGroup in groupedByRelation)
                    {
                        ...
                        var representative = relationGroup
                            .OrderBy(
                                x => BuildBackwardRelationRepresentativeSortKey(x),
                                StringComparer.OrdinalIgnoreCase)
                            .FirstOrDefault();
            ...
            // fallback:
            // 既存 ParentNodes から最小限の candidate を起こす
            if (childNode != null && childNode.ParentNodes != null)
            {
                foreach (var parentNode in childNode.ParentNodes.Where(x => x != null))
                {
                    result.Add(new BackwardParentCandidate
                    {
                        Node = parentNode,
                        ChildLotNumber = childNode.LotNumber,
                        RelationKey = string.Join("|",
                            GetNodeMergeKey(parentNode) ?? string.Empty,
                            "FALLBACK",
                            childNode.LotNumber ?? string.Empty)
                    });
                }
            }
        ...
        private string BuildBackwardRelationRepresentativeSortKey(
    BackwardParentCandidate candidate)
        {
            if (candidate == null || candidate.Node == null)
                return "~~~";
            ...
            string debugSource = string.IsNullOrWhiteSpace(candidate.DebugSource)
                ? string.Empty
                : candidate.DebugSource.Trim();
        ...
        private string BuildBackwardDisplayChildNodeKey(ProductionResultNode node)
        {
            if (node == null)
                return null;
            ...
            if (!string.IsNullOrWhiteSpace(node.ControlMasterKey))
                return node.ControlMasterKey.Trim();

            if (!string.IsNullOrWhiteSpace(node.LotNumber))
                return node.LotNumber.Trim();
        ...
        private string NormalizeBackwardRelationKey(BackwardParentCandidate candidate)
        {
            ...
            string childLot = string.IsNullOrWhiteSpace(candidate.ChildLotNumber)
                ? string.Empty
                : candidate.ChildLotNumber.Trim();
    ...
    string fileName)
        {
            ...
            try
            {
                ...
                sb.AppendLine(
                    "DisplayNodeKey,ParentDisplayNodeKey,MergeKey,XLevel,YLane,ChildIndex,SubtreeLaneSpan,RootGroupHeight,ChildBranchHeight,OccupiedFirstY,OccupiedLastY,LotGroupKey,RepresentativeNodeKey,MasterKey,LotNumber,RouteSystem,InputSlotNo,InputSourceType,Depth,NodeType,RelationKeys");
                    ...
                    .ThenBy(x => x.ChildIndex))
                {
                    ...
                    string relationKeys = null;

                    if (src != null && _currentBackwardCandidatesByChildNodeKey != null)
                    {
                        string childNodeKey = BuildBackwardDisplayChildNodeKey(src);
                        ...
                        if (!string.IsNullOrWhiteSpace(childNodeKey) &&
                            ...
                            candidates.Count > 0)
                        {
                            relationKeys = string.Join("|",
                                candidates
                                    .Where(x => x != null && !string.IsNullOrWhiteSpace(x.RelationKey))
                                    .Select(x => x.RelationKey.Trim())
                                    .Distinct(StringComparer.OrdinalIgnoreCase));
                        }
                    }
                    ...
                    sb.Append(EscapeCsv(relationKeys));
                    sb.AppendLine();
