    ...
    HashSet<string> visitedLinks)
        ...
        private void ResolveStartDateLabelsForDisplayLaneNodes(List<DisplayLaneNode> displayNodes)
        {
            ...
            foreach (var displayNode in displayNodes)
            {
                ...
                // A系(MaterialTableA相当)の開始日時なしは「手投入」
    ...
    List<DisplayLaneNode> displayNodes)
            ...
            int childIndex)
        {
            ...
            return string.Join("|",
                ...
                parent.NodeIdentityKey,
                child.NodeIdentityKey,
                child.LotNumber);
        }
    ...
    IList<OccupiedLotGroupRange> occupiedGroups)
{
    ...
    IList<OccupiedLotGroupRange> occupiedRanges)
        ...
        private bool IsBackwardPreEndOnlyBranchNode(
    DisplayLaneNode node,
    Dictionary<string, List<DisplayLaneNode>> childrenMap,
    int endXLevel)
        {
            ...
            // 子が全部終端なら「終端直前ノード」とみなして branch を作らない
            return children
                .Where(x => x != null)
                .All(x => IsBackwardTerminalDisplayNode(x));
        }
    ...
    string newDisplayNodeKey)
        ...
        private string BuildBackwardEndGroupKeyForUi(DisplayLaneNode node)
        {
            if (node == null || node.SourceNode == null)
                return string.Empty;

            var source = node.SourceNode;

            string masterKey = string.IsNullOrWhiteSpace(source.ControlMasterKey)
                ? string.Empty
                : source.ControlMasterKey.Trim();

            string lotNumber = string.IsNullOrWhiteSpace(source.LotNumber)
                ? string.Empty
                : source.LotNumber.Trim().ToUpperInvariant();

            if (!string.IsNullOrWhiteSpace(masterKey))
            {
                return "END|" + masterKey + "|" + lotNumber;
            }

            return "ENDLOT|" + lotNumber;
        }
