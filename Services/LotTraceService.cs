    ...
    IList<OccupiedLotGroupRange> occupiedGroups)
{
    ...
    int baseY)
        {
            var pathGroupKeys = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

            return PlaceForwardRelationGroupRecursive(
                output,
                relationGroup,
                baseY,
                pathGroupKeys);
    ...
    ForwardRelationGroup group)
        {
            ...
            var parentKeys = group.ParentSet == null
                ? new List<string>()
                : group.ParentSet
                    .Where(x => x != null && !string.IsNullOrWhiteSpace(x.NodeIdentityKey))
                    .Select(x => x.NodeIdentityKey)
            ...
            var childKeys = group.ChildSet == null
                ? new List<string>()
                : group.ChildSet
                    .Where(x => x != null && !string.IsNullOrWhiteSpace(x.NodeIdentityKey))
                    .Select(x => x.NodeIdentityKey)
            ...
            return string.Join(",", parentKeys) + "=>" + string.Join(",", childKeys);
        }
