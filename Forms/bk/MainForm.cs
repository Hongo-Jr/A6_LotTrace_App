    ...
    public partial class MainForm : Form
    {
     ...
     = new Dictionary<string, NodeRenderRange>(StringComparer.OrdinalIgnoreCase);
        ...
        public MainForm(LotTraceService liquidService, BottleTraceService bottleService, ResultService resultService)
        {
            ...
            dataGridStart.CellFormatting += OnNodeKeyGroupForeColorFormatting;
            dataGridMiddle.CellFormatting += OnNodeKeyGroupForeColorFormatting;
            dataGridEnd.CellFormatting += OnNodeKeyGroupForeColorFormatting;
        ...
        private void InitGrids()
{
            ...
            // ▼ 旧2段ヘッダーは完全無効
            // RegisterStartGridTwoRowHeader();
            // RegisterMiddleGridTwoRowHeader();
            // RegisterEndGridTwoRowHeader();
            // RegisterTwoRowHeaderPaintHandlers();
            // ApplyStartGridTwoRowHeaderStyle();
            // ApplyMiddleGridTwoRowHeaderStyle();
            // ApplyEndGridTwoRowHeaderStyle();
        }

        private void ApplyHeaderStyle(DataGridView grid, Color backColor)
        {
        ...
        private void ApplyGridSizingBaseSettings()
        {
            ApplyGridSizingBaseSettings(dataGridStart);
            ApplyGridSizingBaseSettings(dataGridMiddle);
            ApplyGridSizingBaseSettings(dataGridEnd);
            ApplyGridSizingBaseSettings(dataGridIntersection);
        }

        private void ApplyGridSizingBaseSettings(DataGridView grid)
        {
            if (grid == null)
                return;
        ...
        private bool IsLastVisibleMiddleColumnOfLevel(DataGridView grid, int columnIndex)
        {
            if (grid == null)
                return false;
            ...
            DataGridViewColumn currentColumn = grid.Columns[columnIndex];
            if (currentColumn == null || !currentColumn.Visible)
                return false;

            int? currentLevel = GetMiddleHeaderLevelFromColumnName(currentColumn.Name);
    ...
    out bool isOrange)
        ...
        private void DoTrace(TraceSearchParameters p)
        {
            try
            {
                ...
                if (chkExcelTarget.Checked)
                {
                    _excelTargetTabs.Add(tabNo);
                }
                else
                {
        ...
        private sealed class StartRowPayload
        {
            public string StartTrunkGroupKey { get; set; }
            public bool HasMiddleNodes { get; set; }
            public int OriginalRowIndex { get; set; }
            public Dictionary<string, object> StartValues { get; private set; }

            public StartRowPayload()
            {
                StartValues = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase);
            }
        }

        private sealed class LeftStartDisplayRow
        {
            ...
            public LeftStartDisplayRow()
            {
                StartValues = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase);
            }
        }

        private void ApplyLeftGridStartNodeGrouping(DataTable table)
        {
            if (table == null || table.Rows.Count == 0)
                return;

            var startColumns = GetStartColumnNames(table);
            if (startColumns == null || startColumns.Count == 0)
                return;

            var sourceRows = new List<LeftStartDisplayRow>();
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                var item = new LeftStartDisplayRow();
                item.GroupKey = NormalizeStartTrunkGroupKey(row, i);
                item.HasMiddle = RowHasAnyMiddleNode(row);
                item.StartTrunkOrder = GetTableInt(row, "StartTrunkOrder", int.MaxValue);
                item.OriginalRowIndex = i;

                foreach (string colName in startColumns)
                {
                    item.StartValues[colName] = row[colName];
                }

                sourceRows.Add(item);
            }

            // StartTrunkGroupKey 単位で塊化
            // グループ順は「最初に出た順」
            var groupOrder = new List<string>();
            var grouped = new Dictionary<string, List<LeftStartDisplayRow>>(StringComparer.OrdinalIgnoreCase);

            foreach (var item in sourceRows)
            {
                List<LeftStartDisplayRow> bucket;
                if (!grouped.TryGetValue(item.GroupKey, out bucket))
                {
                    bucket = new List<LeftStartDisplayRow>();
                    grouped[item.GroupKey] = bucket;
                    groupOrder.Add(item.GroupKey);
                }

                bucket.Add(item);
            }

            var reorderedLeftStarts = new List<LeftStartDisplayRow>();

            foreach (string groupKey in groupOrder)
            {
                List<LeftStartDisplayRow> bucket;
                if (!grouped.TryGetValue(groupKey, out bucket) || bucket == null || bucket.Count == 0)
                    continue;

                var orderedBucket = bucket
                    .OrderByDescending(x => x.HasMiddle)
                    .ThenBy(x => x.StartTrunkOrder)
                    .ThenBy(x => x.OriginalRowIndex)
                    .ToList();

                reorderedLeftStarts.AddRange(orderedBucket);
            }

            if (reorderedLeftStarts.Count != table.Rows.Count)
                return;

            // 行順はそのまま、Start_* だけ差し替える
            for (int rowIndex = 0; rowIndex < table.Rows.Count; rowIndex++)
            {
                DataRow targetRow = table.Rows[rowIndex];
                LeftStartDisplayRow source = reorderedLeftStarts[rowIndex];

                if (targetRow == null || source == null)
                    continue;

                foreach (string colName in startColumns)
                {
                    object value;
                    if (source.StartValues.TryGetValue(colName, out value))
                    {
                        targetRow[colName] = value ?? DBNull.Value;
                    }
                    else
                    {
                        targetRow[colName] = DBNull.Value;
                    }
                }
            }
        }

        private string NormalizeStartTrunkGroupKey(DataRow row, int rowIndex)
        {
            if (row == null)
                return "__ROW__|" + rowIndex.ToString();

            string key = GetTableString(row, "StartTrunkGroupKey");
            if (!string.IsNullOrWhiteSpace(key))
                return key.Trim();

            string fallback = GetTableString(row, "RootGroupKey");
            if (!string.IsNullOrWhiteSpace(fallback))
                return "__ROOT__|" + fallback.Trim();

            return "__ROW__|" + rowIndex.ToString();
        }

        private int GetTableInt(DataRow row, string columnName, int defaultValue)
        {
        ...
        private List<string> GetStartColumnNames(DataTable table)
        {
            ...
            foreach (DataColumn col in table.Columns)
            {
                ...
                string name = col.ColumnName ?? string.Empty;
        ...
        private bool RowHasAnyMiddleNode(DataRow row)
        {
            ...
            foreach (DataColumn col in row.Table.Columns)
            {
                ...
                if (!name.EndsWith("_NodeKey", StringComparison.OrdinalIgnoreCase))
                    continue;
        ...
        private string GetStartNodeKeyFromBoundRow(DataGridView grid, int rowIndex)
        {
            if (grid == null)
                return null;
            ...
            if (boundItem.Row.Table.Columns.Contains("Start_MasterKey"))
            {
                object mk = boundItem.Row["Start_MasterKey"];
                if (mk != null && mk != DBNull.Value)
                {
                    string text = mk.ToString();
                    if (!string.IsNullOrWhiteSpace(text))
                        return "MK|" + text;
                }
            }

            if (boundItem.Row.Table.Columns.Contains("Start_NodeKey"))
            {
                object nk = boundItem.Row["Start_NodeKey"];
                if (nk != null && nk != DBNull.Value)
                {
                    string text = nk.ToString();
                    if (!string.IsNullOrWhiteSpace(text))
                        return "NK|" + text;
                }
            }

            if (boundItem.Row.Table.Columns.Contains("Start_Lot"))
            {
                object lot = boundItem.Row["Start_Lot"];
                if (lot != null && lot != DBNull.Value)
                {
                    string text = lot.ToString();
                    if (!string.IsNullOrWhiteSpace(text))
                        return "LOT|" + text;
                }
            }
        ...
        private string GetRootGroupKeyFromBoundRow(DataGridView grid, int rowIndex)
        {
            if (grid == null)
                return null;
            ...
            if (!boundItem.Row.Table.Columns.Contains("RootGroupKey"))
                return null;

            object value = boundItem.Row["RootGroupKey"];
            if (value == null || value == DBNull.Value)
                return null;
        ...
        private string GetRouteSystemFromBoundRow(DataGridView grid, int rowIndex)
        {
            if (grid == null)
                return null;
            ...
            if (!boundItem.Row.Table.Columns.Contains("RouteSystem"))
                return null;

            object value = boundItem.Row["RouteSystem"];
            if (value == null || value == DBNull.Value)
                return null;
        ...
        private void AddDisplayNodeColumns(DataTable table, string prefix)
        {
            if (table == null || string.IsNullOrWhiteSpace(prefix))
                return;

            // 内部列
            table.Columns.Add(prefix + "_MasterKey");
            table.Columns.Add(prefix + "_NodeKey");
            table.Columns.Add(prefix + "_ParentKey");
            table.Columns.Add(prefix + "_DisplayParentNodeKey");
            table.Columns.Add(prefix + "_IncomingLinkKey");
            table.Columns.Add(prefix + "_UpstreamPathKey");
            table.Columns.Add(prefix + "_DownstreamPathKey");

            // 表示列
            table.Columns.Add(prefix + "_Order");
            table.Columns.Add(prefix + "_Lot");
            table.Columns.Add(prefix + "_ItemCode");
            table.Columns.Add(prefix + "_ItemName");
            table.Columns.Add(prefix + "_StartTime");
            table.Columns.Add(prefix + "_Weight", typeof(decimal));
        }

        private void SetDisplayCellColumns(DataRow row, string prefix, TraceDisplayCell cell)
        {
            ...
            if (cell == null)
            {
                row[prefix + "_MasterKey"] = DBNull.Value;
                row[prefix + "_NodeKey"] = DBNull.Value;
                row[prefix + "_ParentKey"] = DBNull.Value;
                row[prefix + "_DisplayParentNodeKey"] = DBNull.Value;
                row[prefix + "_IncomingLinkKey"] = DBNull.Value;
                row[prefix + "_UpstreamPathKey"] = DBNull.Value;
                row[prefix + "_DownstreamPathKey"] = DBNull.Value;
                row[prefix + "_Order"] = DBNull.Value;
                row[prefix + "_Lot"] = DBNull.Value;
                row[prefix + "_ItemCode"] = DBNull.Value;
                row[prefix + "_ItemName"] = DBNull.Value;
                row[prefix + "_StartTime"] = DBNull.Value;
                row[prefix + "_Weight"] = DBNull.Value;
                return;
            }

            // 内部列は suppress されても残す
            row[prefix + "_MasterKey"] = ToDbValue(cell.MasterKey);
            row[prefix + "_NodeKey"] = ToDbValue(cell.NodeKey);
            row[prefix + "_ParentKey"] = ToDbValue(cell.ParentKey);
            row[prefix + "_DisplayParentNodeKey"] = ToDbValue(cell.DisplayParentNodeKey);
            row[prefix + "_IncomingLinkKey"] = ToDbValue(cell.IncomingLinkKey);
            row[prefix + "_UpstreamPathKey"] = ToDbValue(cell.UpstreamPathKey);
            row[prefix + "_DownstreamPathKey"] = ToDbValue(cell.DownstreamPathKey);

            // 表示列は IsVisible に従う
            if (cell.IsVisible)
            {
                row[prefix + "_Order"] = ToDbValue(cell.ProductionOrderNumber);
                row[prefix + "_Lot"] = ToDbValue(cell.LotNumber);
                row[prefix + "_ItemCode"] = ToDbValue(cell.ItemCode);
                row[prefix + "_ItemName"] = ToDbValue(cell.ItemName);
                row[prefix + "_StartTime"] = ToDbValue(GetDisplayStartTimeText(cell));
                row[prefix + "_Weight"] = cell.Weight.HasValue
                    ? (object)cell.Weight.Value
                    : DBNull.Value;
            }
            else
            {
        ...
        private string GetDisplayStartTimeText(TraceDisplayCell cell)
        {
            if (cell == null)
                return null;

            if (!string.IsNullOrWhiteSpace(cell.StartDateLabel))
                return cell.StartDateLabel;

            return cell.StartDate.HasValue
                ? cell.StartDate.Value.ToString("yyyy/MM/dd HH:mm:ss")
                : null;
        }

        private string GetDisplayStartTimeText(ProductionResultNode node)
        {
            if (node == null)
                return null;
            ...
            return node.StartDate.HasValue
    ...
    /// <param name="groupRows"></param>
    /// <param name="visitedEdges"></param>
        private void BuildRowsFromGraph(
    ProductionResultNode node,
    DataTable table,
    List<ProductionResultNode> path,
    List<DataRow> groupRows,
    HashSet<string> visitedEdges)
        {
            ...
            if (children.Count == 0)
            {
                ...
                var start = path.FirstOrDefault();
                var end = path.LastOrDefault();
                var middles = path.Skip(1).Take(path.Count - 2).ToList();

                // Start
                if (start != null)
                {
                    row["Start_MasterKey"] = start.ControlMasterKey ?? (object)DBNull.Value;
                    row["Start_Order"] = start.ProductionOrderNumber ?? (object)DBNull.Value;
                    row["Start_Lot"] = start.LotNumber ?? (object)DBNull.Value;
                    row["Start_ItemCode"] = start.ItemCode ?? (object)DBNull.Value;
                    row["Start_ItemName"] = start.ItemName ?? (object)DBNull.Value;
                    row["Start_StartTime"] = ToDbValue(GetDisplayStartTimeText(start));
                    row["Start_Weight"] = start.Weight.HasValue ? (object)start.Weight.Value : DBNull.Value;
                    row["Start_NodeKey"] = GetNodeIdentityForUi(start) ?? (object)DBNull.Value;
                    row["Start_ParentKey"] = DBNull.Value;
                }
                ...
                for (int i = 0; i < middles.Count; i++)
                {
                    ...
                    EnsureMiddleColumns(table, lv);

                    row["Lv" + lv + "_Order"] = m.ProductionOrderNumber ?? (object)DBNull.Value;
                    row["Lv" + lv + "_Lot"] = m.LotNumber ?? (object)DBNull.Value;
                    row["Lv" + lv + "_ItemCode"] = m.ItemCode ?? (object)DBNull.Value;
                    row["Lv" + lv + "_ItemName"] = m.ItemName ?? (object)DBNull.Value;
                    row["Lv" + lv + "_StartTime"] = ToDbValue(GetDisplayStartTimeText(m));
                    row["Lv" + lv + "_Weight"] = m.Weight.HasValue ? (object)m.Weight.Value : DBNull.Value;
                    row["Lv" + lv + "_MasterKey"] = m.ControlMasterKey ?? (object)DBNull.Value;
                    row["Lv" + lv + "_NodeKey"] = GetNodeIdentityForUi(m) ?? (object)DBNull.Value;
                    row["Lv" + lv + "_ParentKey"] = GetFirstParentIdentityForUi(m) ?? (object)DBNull.Value;
                }

                // End
                if (end != null)
                {
                    row["End_MasterKey"] = end.ControlMasterKey ?? (object)DBNull.Value;
                    row["End_Order"] = end.ProductionOrderNumber ?? (object)DBNull.Value;
                    row["End_Lot"] = end.LotNumber ?? (object)DBNull.Value;
                    row["End_ItemCode"] = end.ItemCode ?? (object)DBNull.Value;
                    row["End_ItemName"] = end.ItemName ?? (object)DBNull.Value;
                    row["End_StartTime"] = ToDbValue(GetDisplayStartTimeText(end));
                    row["End_Weight"] = end.Weight.HasValue ? (object)end.Weight.Value : DBNull.Value;
                    row["End_NodeKey"] = GetNodeIdentityForUi(end) ?? (object)DBNull.Value;
                    row["End_ParentKey"] = GetFirstParentIdentityForUi(end) ?? (object)DBNull.Value;
                }
            ...
            {
                string edgeKey = GetGraphEdgeKey(node, child);
        ...
        private string GetNodeIdentityForUi(ProductionResultNode node)
        {
            if (node == null)
                return null;
        ...
        private string GetFirstParentIdentityForUi(ProductionResultNode node)
        {
            if (node == null || node.ParentNodes == null || node.ParentNodes.Count == 0)
                return null;

            return GetNodeIdentityForUi(node.ParentNodes[0]);
        }

        private string GetGraphEdgeKey(ProductionResultNode parent, ProductionResultNode child)
        {
            string parentKey = GetNodeIdentityForUi(parent) ?? "";
            string childKey = GetNodeIdentityForUi(child) ?? "";
            return parentKey + "->" + childKey;
        }


        private int GetMiddleDepth(TraceDisplayRow displayRow)
        {
            if (displayRow == null || displayRow.Middles == null)
                return 0;

            return displayRow.Middles.Count(m => m != null);
        }
        

        private int GetMaxMiddleDepthFromNode(ProductionResultNode node, HashSet<string> visited)
        {
            if (node == null)
                return 0;

            string key = GetNodeIdentityForUi(node) ?? Guid.NewGuid().ToString();
            ...
            {
                int d = GetMaxMiddleDepthFromNode(child, visited);
                if (d > maxChildDepth)
                    maxChildDepth = d;
            }
            ...
            // root は Middle に含めないので +1 は child 側で表現
    ...
    //    private void BuildFullPaths(
    //ProductionResultNode node,
    //List<ProductionResultNode> allNodes,
    //List<ProductionResultNode> path,
    //DataTable table,
    //int maxDepth)
    //    {
    ...
    //        if (path.Any(p => string.Equals(p.LotNumber, node.LotNumber, StringComparison.OrdinalIgnoreCase)))
    //            return;
    ...
    //            .Where(n =>
    //                !string.IsNullOrWhiteSpace(n.ParentKey) &&
    //                !string.IsNullOrWhiteSpace(node.LotNumber) &&
    //                string.Equals(n.ParentKey, node.LotNumber, StringComparison.OrdinalIgnoreCase))
    //            .ToList();
    ...
    //            var start = path.FirstOrDefault(n => n.NodeType == "Start");
    //            var end = path.LastOrDefault(n => n.NodeType == "End");
    //            var middles = path.Where(n => n.NodeType == "Middle").ToList();
    ...
    //                row["Start_ItemCode"] = start.ItemCode ?? (object)DBNull.Value;
    //                row["Start_ItemName"] = start.ItemName ?? (object)DBNull.Value;
    //                row["Start_StartTime"] = start.StartDate.HasValue ? (object)start.StartDate.Value : DBNull.Value;
    //                row["Start_Weight"] = start.Weight.HasValue ? (object)start.Weight.Value : DBNull.Value;

    //                row["Start_NodeKey"] = start.LotNumber ?? (object)DBNull.Value;
    //                row["Start_ParentKey"] = start.ParentKey ?? (object)DBNull.Value;
    //            }
    ...
    //                row["Lv" + lv + "_ItemCode"] = m.ItemCode ?? (object)DBNull.Value;
    //                row["Lv" + lv + "_ItemName"] = m.ItemName ?? (object)DBNull.Value;
    //                row["Lv" + lv + "_StartTime"] = m.StartDate.HasValue ? (object)m.StartDate.Value : DBNull.Value;
    //                row["Lv" + lv + "_Weight"] = m.Weight.HasValue ? (object)m.Weight.Value : DBNull.Value;

    //                row["Lv" + lv + "_NodeKey"] = m.LotNumber ?? (object)DBNull.Value;
    //                row["Lv" + lv + "_ParentKey"] = m.ParentKey ?? (object)DBNull.Value;
    //            }
    ...
    //                row["End_ItemCode"] = end.ItemCode ?? (object)DBNull.Value;
    //                row["End_ItemName"] = end.ItemName ?? (object)DBNull.Value;
    //                row["End_StartTime"] = end.StartDate.HasValue ? (object)end.StartDate.Value : DBNull.Value;
    //                row["End_Weight"] = end.Weight.HasValue ? (object)end.Weight.Value : DBNull.Value;

    //                row["End_NodeKey"] = end.LotNumber ?? (object)DBNull.Value;
    //                row["End_ParentKey"] = end.ParentKey ?? (object)DBNull.Value;
    //            }

    //            bool exists = table.AsEnumerable()
    //                .Any(r => Convert.ToString(r["Start_Order"]) == (start == null ? null : start.ProductionOrderNumber));

    //            if (exists)
    //            {
    //                row["Start_Order"] = DBNull.Value;
    //                row["Start_Lot"] = DBNull.Value;
    //                row["Start_ItemCode"] = DBNull.Value;
    //                row["Start_ItemName"] = DBNull.Value;
    //                row["Start_StartTime"] = DBNull.Value;
    //                row["Start_Weight"] = DBNull.Value;
    //            }
    ...
    //            foreach (var child in children)
    //            {
    //                BuildFullPaths(child, allNodes, new List<ProductionResultNode>(path), table, maxDepth);
    //            }
    //        }
    //    }

    //    private void BuildMiddlePaths(
    //ProductionResultNode node,
    //List<ProductionResultNode> middleNodes,
    //List<ProductionResultNode> path,
    //DataTable table)
    //    {
    //        if (node == null)
    ...
    //        if (path == null)
    //            path = new List<ProductionResultNode>();
    ...
    //                row["Lv" + level + "_指図番号"] = path[i].ProductionOrderNumber ?? (object)DBNull.Value;
    //                row["Lv" + level + "_ロット"] = path[i].LotNumber ?? (object)DBNull.Value;
    //                row["Lv" + level + "_品目コード"] = path[i].ItemCode ?? (object)DBNull.Value;
    //                row["Lv" + level + "_品目名"] = path[i].ItemName ?? (object)DBNull.Value;
    //                row["Lv" + level + "_開始日時"] = path[i].StartDate.HasValue
    //                    ? (object)path[i].StartDate.Value
    //                    : DBNull.Value;
    //                row["Lv" + level + "_重量"] = path[i].Weight.HasValue
    //                    ? (object)path[i].Weight.Value
    //                    : DBNull.Value;
    //            }
    ...
    //        else
    //        {
    //            foreach (ProductionResultNode child in children)
    //            {
    //                BuildMiddlePaths(
    //                    child,
    //                    middleNodes,
    //                    new List<ProductionResultNode>(path),
    //                    table);
    //            }
    //        }
    //    }

        private void EnsureMiddleColumns(DataTable table, int lv)
        {
            string prefix = "Lv" + lv + "_";

            if (!table.Columns.Contains(prefix + "Order"))
                table.Columns.Add(prefix + "Order");

            if (!table.Columns.Contains(prefix + "Lot"))
                table.Columns.Add(prefix + "Lot");

            if (!table.Columns.Contains(prefix + "ItemCode"))
                table.Columns.Add(prefix + "ItemCode");

            if (!table.Columns.Contains(prefix + "ItemName"))
                table.Columns.Add(prefix + "ItemName");

            if (!table.Columns.Contains(prefix + "StartTime"))
                table.Columns.Add(prefix + "StartTime");

            if (!table.Columns.Contains(prefix + "Weight"))
                table.Columns.Add(prefix + "Weight", typeof(decimal));

            if (!table.Columns.Contains(prefix + "MasterKey"))
                table.Columns.Add(prefix + "MasterKey");

            if (!table.Columns.Contains(prefix + "NodeKey"))
                table.Columns.Add(prefix + "NodeKey");

            if (!table.Columns.Contains(prefix + "ParentKey"))
                table.Columns.Add(prefix + "ParentKey");
        }
        ...
        private string GetStartTrunkGroupKeyFromBoundRow(DataGridView grid, int rowIndex)
        {
            if (grid == null)
                return null;
            ...
            var boundItem = grid.Rows[rowIndex].DataBoundItem as DataRowView;
            if (boundItem == null || boundItem.Row == null || boundItem.Row.Table == null)
                return null;

            if (!boundItem.Row.Table.Columns.Contains("StartTrunkGroupKey"))
                return null;

            object value = boundItem.Row["StartTrunkGroupKey"];
            if (value == null || value == DBNull.Value)
                return null;
        ...
        private bool ShouldDrawMiddleTreeDividerByLinkContext(MiddleLinkContext context, int rowIndex)
        {
            if (context == null)
                return false;

            if (context.SiblingCount <= 1)
                return false;

            if (!context.LastSiblingRowIndex.HasValue)
                return false;

            // 「今行の下に線を引く」判定なので、
            // 最後の兄弟行の1つ手前まで true
            return rowIndex < context.LastSiblingRowIndex.Value;
        }

        private bool ShouldDrawMiddleVerticalLine(NodeRenderRange parentRange, int rowIndex)
        {
            if (parentRange == null)
                return false;

            if (parentRange.ChildCount <= 1)
                return false;

            if (!parentRange.FirstChildRowIndex.HasValue || !parentRange.LastChildRowIndex.HasValue)
                return false;

            return rowIndex >= parentRange.FirstChildRowIndex.Value
                && rowIndex < parentRange.LastChildRowIndex.Value;
        }

        private bool ShouldDrawMiddleBranchStart(NodeRenderRange parentRange, int rowIndex)
        {
            if (parentRange == null)
                return false;
            ...
            if (!parentRange.FirstChildRowIndex.HasValue)
                return false;

            return rowIndex == parentRange.FirstChildRowIndex.Value;
        }

        private bool ShouldDrawMiddleSingleConnector(NodeRenderRange parentRange, int rowIndex)
        {
            if (parentRange == null)
                return false;
        ...
        private bool IsLastChildRow(NodeRenderRange parentRange, int rowIndex)
        {
            if (parentRange == null)
                return false;

            if (!parentRange.LastChildRowIndex.HasValue)
                return false;

            return rowIndex == parentRange.LastChildRowIndex.Value;
        }
        ...
        private bool ShouldDrawPathGroupDivider(DataGridView grid, int rowIndex)
        {
            return ShouldDrawDividerByPathKey(grid, rowIndex);
        }

        private bool ShouldDrawDividerByPathKey(DataGridView grid, int rowIndex)
        {
            if (grid == null)
                return false;
            ...
            var next = grid.Rows[rowIndex + 1];

            if (row.IsNewRow || next.IsNewRow)
                return false;

            string currentPathKey = GetPathKeyFromBoundRow(grid, rowIndex);
            string nextPathKey = GetPathKeyFromBoundRow(grid, rowIndex + 1);

            if (string.IsNullOrWhiteSpace(currentPathKey) ||
                string.IsNullOrWhiteSpace(nextPathKey))
                return false;

            return !string.Equals(currentPathKey, nextPathKey, StringComparison.OrdinalIgnoreCase);
        }
        ...
        private void btnDetectCrossPoints_Click(object sender, EventArgs e)
        {
            ...
            tcMainTabs.SelectedTab = tpIntersection;
        }
        ...
        private void ExportDisplayRowsCsv(DataTable table)
        {
            ExportDebugDataTableToCsv(table, "LotTrace_DisplayRows");
        }

        private void ExportDebugRowsWithSuppressionCsv(DataTable table)
        {
            ExportDebugDataTableToCsv(table, "LotTrace_DebugRows_WithSuppression");
        }
    ...
    TraceDisplayResult displayResult)
        {
            ...
            foreach (var line in displayResult.LineRanges)
            {
                ...
                if (!string.Equals(line.GridKind, "Middle", StringComparison.OrdinalIgnoreCase))
                    continue;
    ...
    TraceDisplayResult displayResult)
        ...
        {
            return tcSearchSlots == null ? 0 : tcSearchSlots.SelectedIndex + 1;
        }
        ...
        private bool HasMiddleHorizontalLineOnRow(int rowIndex)
        {
            var lines = GetMiddleHorizontalLinesForRow(rowIndex);
        ...
        private bool IsTrunkLineKind(string lineKind)
        {
            return string.Equals(lineKind, "Trunk", StringComparison.OrdinalIgnoreCase);
        }

        private bool IsBranchLineKind(string lineKind)
        {
            return string.Equals(lineKind, "Branch", StringComparison.OrdinalIgnoreCase);
        }
            ...
            List<MiddleHorizontalLineDrawInfo> lines)
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (IsTrunkLineKind(line.LineKind))
                    return line;

                if (firstBranch == null && IsBranchLineKind(line.LineKind))
                    firstBranch = line;
            }
    ...
    MiddleHorizontalLineDrawInfo line)
        ...
        {
            if (string.Equals(lineKind, "Trunk", StringComparison.OrdinalIgnoreCase))
                return Color.Orange;

            if (string.Equals(lineKind, "Branch", StringComparison.OrdinalIgnoreCase))
                return Color.Black;
        ...
        private string ResolvePreferredLineKind(IEnumerable<string> lineKinds)
        {
            ...
            {
                if (IsTrunkLineKind(lineKind))
                    return "Trunk";

                if (IsBranchLineKind(lineKind))
                    hasBranch = true;
            }

            return hasBranch ? "Branch" : null;
        }
        ...
        private bool HasEndHorizontalLineOnRow(int rowIndex)
        {
            var lines = GetEndHorizontalLinesForRow(rowIndex);
            return lines != null && lines.Count > 0;
        }
    ...
    MiddleVerticalLineDrawInfo line)
        ...
        private void RegisterStartGridTwoRowHeader()
        {
            RegisterTwoRowHeaderPainter(dataGridStart, dataGridStart_TwoRowHeaderCellPainting);
        }

        private void RegisterMiddleGridTwoRowHeader()
        {
            RegisterTwoRowHeaderPainter(dataGridMiddle, dataGridMiddle_TwoRowHeaderCellPainting);
        }

        private void RegisterEndGridTwoRowHeader()
        {
            RegisterTwoRowHeaderPainter(dataGridEnd, dataGridEnd_TwoRowHeaderCellPainting);
        }

        private void ApplyStartGridTwoRowHeaderStyle()
        {
            ApplyTwoRowHeaderStyle(dataGridStart, _startHeaderStyle);
        }

        private void ApplyMiddleGridTwoRowHeaderStyle()
        {
            ApplyTwoRowHeaderStyle(dataGridMiddle, _middleHeaderStyle);
        }

        private void ApplyEndGridTwoRowHeaderStyle()
        {
            ApplyTwoRowHeaderStyle(dataGridEnd, _endHeaderStyle);
        }
        ...
        private void dataGridStart_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            using (var f = new LotTraceApp.Forms.Result(
                _resultService, productionOrderNumber, itemCode, itemName, lotNumber, ""))
            {
                f.ShowDialog(this);
            }
        }
