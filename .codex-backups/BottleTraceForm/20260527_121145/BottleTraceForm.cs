...
namespace LotTraceApp
{
    public partial class BottleTraceForm : Form
    {
        ...
        private sealed class LineCache
        {
            ...
            public int LiquidWidth { get; set; }
            public int BottleWidth { get; set; }
        } 
        ...
        private void LiqidTableBorderPaint(object sender, PaintEventArgs e)
        {
            ...
            foreach(var cache in _lineCache)
            {
                ...
                Rectangle rect = grid.GetRowDisplayRectangle(cache.RowIndex, true);
                if (rect.Height <= 0)
                    continue;
                ...
                using (var pen = new Pen(cache.Color, cache.LiquidWidth))
                {
                    e.Graphics.DrawLine(
                        pen,
                        left,
        ...
        private void BottleTableBorderPaint(object sender, PaintEventArgs e)
        {
            ...
            foreach (var cache in _lineCache)
            {
                ...
                using (var pen = new Pen(cache.Color, cache.BottleWidth))
                {
                    e.Graphics.DrawLine(
                        pen,
                        left,
